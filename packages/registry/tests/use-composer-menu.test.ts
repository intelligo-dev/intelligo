...
describe("detectTrigger", () => {
  it("opens the command list for a slash that starts the message", () => {
    ...
    expect(detectTrigger("/sum", 4)).toEqual({
      kind: "command",
      start: 0,
      query: "sum",
    });
  ...
  it("opens the mention picker for an @ that starts a word", () => {
    expect(detectTrigger("ask @doc", 8)).toEqual({
      kind: "mention",
      start: 4,
      query: "doc",
    });
  ...
  it("honours a custom mention trigger and the caret position", () => {
    expect(detectTrigger("see #q3", 7, "#")).toEqual({
      kind: "mention",
      start: 4,
      query: "q3",
    });
    expect(detectTrigger("see @q3 and more", 7)).toEqual({
      kind: "mention",
      start: 4,
      query: "q3",
    });
