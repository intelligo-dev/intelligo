...
const GROUPS: Record<RegistryGroup, string[]> = {
  Auth: ["auth-login", "auth-signup", "auth-password-reset", "auth-email-verification", "onboarding", "invitation-accept"],
  Shell: ["app-shell", "dashboard", "notifications", "language-switcher", "trial-banner", "route-error"],
  Settings: ["settings-shell", "workspace-settings", "team-settings", "profile-settings", "privacy-settings"],
  Commerce: ["pricing", "checkout", "billing-settings", "usage", "feature-gating", "payment-poll"],
  AI: ["chat", "artifacts"],
};
...
const raw = (registry as { items: RawItem[] }).items.filter((i) => i.name !== "smoke");

export const REGISTRY_ITEMS: RegistryItem[] = (Object.keys(GROUPS) as RegistryGroup[]).flatMap((group) =>
  GROUPS[group]
