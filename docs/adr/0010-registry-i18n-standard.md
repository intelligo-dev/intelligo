...
**Driver:** The first product built on the framework must consume Intelligo entirely through packages and the registry — no customized forks — so that every future product starts from the same standard surface.

## Context

The first registry standard shipped items with plain-English copy and no
i18n dependency, and allowed the proof product to keep customized copies of the UI.
...
time — by hand — in the proof product's diverged copy. The founder's direction is
...
2. **Each item ships its English messages** as a registry file targeting
   ...
   files (`messages/<locale>/<item>.json`) — translation is configuration,
...
3. **Targets are locale-aware.** Page files target
   ...
   `[locale]` segment and next-intl request wiring, so the proof product's existing
...
- The proof product installs registry items verbatim, contributes `messages/<locale>/*`
