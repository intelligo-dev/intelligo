...
**Unrecorded usage is never reported as success.** `complete()` first claims the row (`running → settling`) and only then calls `settleUsage`. If that throws, it emits `execution.settlement_failed`, leaves the row `settling`, and rethrows. A `running` or `settling` row past the route's `maxDuration` is the operational signal that something went wrong between the model and the ledger — `findStaleExecutions` and the admin console's settlement probe report them; nothing repairs them automatically. Note that `settling` has two readings: the charge never happened, or it committed and the process died before the final `settling → succeeded` flip — the `usage_records` / `credit_reservations` rows for the same `requestId` tell them apart. The alternative — marking it succeeded with no charge — makes a revenue leak indistinguishable from a free turn.
