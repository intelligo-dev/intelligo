...
export const RULES: Rule[] = [
  {
    ...
    passing: "✓ imports only allowlisted @intelligo-dev/* packages and no app code",
    violation: "import { credits } from \"@intelligo-dev/billing\"  // inside packages/auth",
    output: [
  ...
  },
  {
    ...
    violation: "db.select().from(facts).where(eq(facts.userId, userId))  // no workspaceId",
    output: [
