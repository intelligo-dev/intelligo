...
- **`apps/site`** — intelligo.dev: the framework's public site (Astro static + React islands) and the **hosted page registry** at `https://intelligo.dev/r/<item>.json`. Everything it says about the framework is generated by `pnpm --filter site sync` from this tree (registry items, counts, the reference app's primitives) and committed, so it cannot describe a page that does not exist. Deployed as static assets on Cloudflare Workers (root directory `apps/site`).
