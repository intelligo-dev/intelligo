...
function stubChatModel(): LanguageModel {
  return new MockLanguageModelV3({
    ...
    doStream: async ({ prompt }) => {
      ...
      // One call per turn: once the transcript carries a saveArtifact
      // call, the follow-up step reports instead of calling again.
      const alreadyCalledTool = JSON.stringify(prompt).includes("saveArtifact");
      const callsTool = TOOL_TRIGGER.test(userText) && !alreadyCalledTool;

      const reply = callsTool
      ...
      const chunks: StubStreamPart[] = [
        ...
        ...(callsTool
          ? ([
            ...
            ] satisfies StubStreamPart[])
          : []),
        {
          type: "finish",
          finishReason: callsTool
