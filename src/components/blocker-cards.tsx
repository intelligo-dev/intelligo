...
function Card({
  ...
  const scale = useTransform(scrollYProgress, [start, 1], reduce ? [1, 1] : [1, restingScale]);

  return (
    // later cards must paint over earlier ones: explicit z-index, opaque surfaces
    <section className="sticky top-20 flex h-[62vh] min-h-[420px] items-start justify-center" style={{ zIndex: index + 1 }}>
      <motion.article
      ...
      >
        <div>
          ...
          <h3 className="mt-3 text-[1.35rem] font-semibold leading-tight text-ink md:text-[1.6rem]">{card.title}</h3>
          <p className="mt-3 max-w-[46ch] text-[0.98rem] text-ink-dim">{card.body}</p>
        </div>
        <div className="mono self-end border border-line bg-paper-sunken p-3.5 text-[0.76rem] leading-relaxed">
          {card.evidence.map((l, i) => (
            <div key={i} className={l.startsWith("$") ? "text-ink" : l.startsWith("✓") ? "text-settle" : "text-ink-dim"}>
              {l}
            </div>
          ))}
        </div>
...
export function BlockerCards() {
  ...
  return (
    <div ref={container} className="relative">
      {BLOCKERS.map((b, i) => (
        <Card key={b.tag} card={b} index={i} total={BLOCKERS.length} container={container} reduce={reduce} />
      ))}
    </div>
  );
}
