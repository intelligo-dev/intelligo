...
export const SCENES = [
  { id: "build", n: "01", title: "The build", note: "Pages are created group by group, the packages install, and the parts lock into one running application." },
  { id: "run", n: "02", title: "The run", note: "A message goes in. Intelligo admits it, your agent answers natively, the run settles — credits, usage and audit move in the same frame." },
  { id: "yours", n: "03", title: "Make it yours", note: "The chat page is your source. Change one line of config and the running app follows; the components stay untouched." },
  { id: "time", n: "04", title: "Time passes", note: "A new release lands. Migrations ship with it, the doctor sees what you customised and keeps it. Still yours." },
] as const;
...
const STEPS: Step[] = [
  ...MOMENTS.map((m, k) => ({ scene: 0, label: `${m.title} created`, at: g(SCENE.build, momentWindow(k)[0]) })),
  { scene: 0, label: "Packages installed", at: g(SCENE.build, ACT.shelve[0]) },
  { scene: 0, label: "SaaS ready", at: g(SCENE.build, ACT.clear[0]) },
  { scene: 0, label: "Your agent lands", at: g(SCENE.build, ACT.app[0] + 0.8 * (ACT.app[1] - ACT.app[0])) },
  { scene: 1, label: "A message is typed", at: g(SCENE.run, 0) },
  { scene: 1, label: "Admit — entitlement checked, credits reserved", at: g(SCENE.run, 0.2) },
  { scene: 1, label: "Run — your agent, unmodified", at: g(SCENE.run, 0.42) },
  { scene: 1, label: "Settle — usage recorded, credits charged", at: g(SCENE.run, 0.68) },
  { scene: 2, label: "Open the installed page", at: g(SCENE.yours, 0) },
  { scene: 2, label: "Edit lib/chat-config.tsx", at: g(SCENE.yours, 0.25) },
  { scene: 2, label: "Hot reload — components untouched", at: g(SCENE.yours, 0.62) },
...
const G = (() => {
  const SB = 176, TB = 44, PAD = 18, gap = 12;
  const cx = SB + PAD, cy = TB + PAD, cw = W - cx - PAD;
  const col3 = (cw - gap * 2) / 3, r1h = 118;
  const r2y = cy + r1h + gap, r2h = H - r2y - PAD;
  const chatW = Math.round(cw * 0.62);
  const rightX = cx + chatW + gap, rightW = cw - chatW - gap;
  const opsH = Math.round((r2h - gap) * 0.52);
  return { SB, TB, PAD, gap, cx, cy, cw, col3, r1h, r2y, r2h, chatW, rightX, rightW, opsH };
})();
...
function useTyped(mv: MotionValue<number>, a: number, b: number, text: string) {
  return useTransform(mv, (v) => text.slice(0, Math.round(seg(v, a, b) * text.length)));
}

function MRow({ l, r, color, opacity }: { l: string; r: MotionValue<string>; color?: MotionValue<string>; opacity?: MotionValue<number> }) {
  return (
    <motion.div style={{ opacity }} className="mono flex items-center justify-between border-t border-line py-[3px] text-[9.5px] first:border-t-0">
      <span className="text-ink-dim">{l}</span>
...
type Live = { p: MotionValue<number>; run: MotionValue<number>; yours: MotionValue<number>; time: MotionValue<number> };

const ASK = "Which clauses expose us at renewal?";
const ANSWER = "Two: the auto-renewal in §4.2 (90-day notice, already inside the window) and the price-escalator in §7 with no cap. Recommend serving notice this week.";

function Stage({ p, run, yours, time }: Live) {
  const { SB, TB, cx, cy, col3, r1h, r2y, r2h, chatW, rightX, rightW, opsH, gap } = G;
  ...
  const runTag = useTransform<number, string>(run, (v) => (v < 0.7 ? "streaming · 14 reserved" : "settled · 14 credits"));
  const runTagColor = useTransform<number, string>(run, (v) => (v < 0.7 ? "var(--amber)" : "var(--settle)"));
  const credits = useTransform<number, string>(run, (v) => (v < 0.72 ? "1,240" : "1,226"));
  const creditsRow = useTransform<number, string>(run, (v) => (v < 0.72 ? "1,240 / 2,000" : "1,226 / 2,000"));
  const reserved = useTransform<number, string>(run, (v) => (v < 0.22 ? "38 (2 runs)" : v < 0.72 ? "52 (3 runs)" : "38 (2 runs)"));
  const reservedColor = useTransform<number, string>(run, (v) => (v >= 0.22 && v < 0.72 ? "var(--amber)" : "var(--ink)"));
  const newRun = useTransform<number, string>(run, (v) => (v < 0.7 ? "admitted · 14 cr held" : "settled · 14 cr"));
  const newRunColor = useTransform<number, string>(run, (v) => (v < 0.7 ? "var(--amber)" : "var(--settle)"));
  ...
  const chatTitle = useTransform<number, string>(yours, (v) => (v < 0.5 ? "Chat" : "Contract Copilot"));
  const placeholder = useTransform<number, string>(yours, (v) => (v < 0.56 ? "Ask anything…" : "Ask about any clause…"));
  const toastOp = useTransform(yours, [0.62, 0.66, 0.9, 0.95], [0, 1, 1, 0]);

  // scene 4 — time passes
  const version = useTransform<number, string>(time, (v) => (v < 0.7 ? PACKAGE_VERSION : NEXT_VERSION));
  const versionColor = useTransform<number, string>(time, (v) => (v >= 0.7 && v < 0.9 ? "var(--settle)" : "var(--ink-faint)"));
    ...
    <div className="relative" style={{ width: W, height: H }}>
      ...
      <Part p={p} from={{ x: -260 }} win={[0.06, 0.2]} style={{ left: 0, top: 0, width: SB, height: H }} className="rounded-l-xl border-r border-line bg-paper-sunken/60">
        <div className="flex h-full flex-col p-3">
          ...
          <div className="mt-auto rounded-sm border border-line bg-paper p-2">
            <div className="mono flex justify-between text-[9px] text-ink-faint"><span>credits</span><motion.span className="text-ink">{credits}</motion.span></div>
      ...
      <Part p={p} from={{ x: 320, y: -180, r: 7 }} win={[0.4, 0.54]} style={{ left: cx + (col3 + gap) * 2, top: cy, width: col3, height: r1h }}>
        <Tile title="Executions · today" icon={Activity} src="executions">
          <MRow l="chat.message" r={newRun} color={newRunColor} opacity={newRunOp} />
      ...
      <Part p={p} from={{ y: 320, s: 0.92 }} win={[0.48, 0.64]} style={{ left: cx, top: r2y, width: chatW, height: r2h }}>
        <Tile title={<motion.span>{chatTitle}</motion.span>} icon={MessageSquare} src="chat · core" className="relative">
          <div className="flex h-full flex-col gap-1.5 overflow-hidden">
            ...
            <motion.div style={{ scaleY: oldAnswerH, opacity: oldAnswerH, transformOrigin: "top" }} className="max-w-[86%] rounded-md border border-line bg-paper-raised px-2.5 py-1.5 text-[10px] text-ink">
            ...
            <motion.div style={{ opacity: askOp }} className="max-w-[78%] self-end rounded-md bg-ink px-2.5 py-1.5 text-[10px] text-paper">
              {ASK}
            </motion.div>
            <motion.div style={{ opacity: answerOp }} className="max-w-[92%] rounded-md border border-line bg-paper-raised px-2.5 py-1.5 text-[10px] text-ink">
              <motion.span>{answer}</motion.span>
              <div className="mono mt-1 flex gap-1 text-[8.5px] text-ink-faint">
                <motion.span style={{ color: runTagColor, borderColor: runTagColor }} className="inline-block rounded-sm border px-1.5 py-px text-[9px] leading-tight">
                  {runTag}
                </motion.span>
                <Tag>gpt-5-mini</Tag>
              </div>
            ...
            <div className="relative mt-auto">
              ...
              <motion.div style={{ opacity: agentIn, scale: agentScale, y: agentY }} className="relative will-change-transform">
                <div className="flex items-center gap-2.5 rounded-md border border-dashed border-amber bg-amber-soft/50 px-3 py-2">
                  <span className="flex size-6 items-center justify-center rounded-md bg-ink text-paper"><Bot className="size-3.5" strokeWidth={1.75} /></span>
                  <span>
                    <span className="block text-[10.5px] font-semibold text-ink">your agent</span>
                    <span className="mono block text-[8px] text-ink-dim">prompts · tools · knowledge — Mastra, AI SDK, anything</span>
                  </span>
                  <span className="mono ml-auto text-[8px] text-amber">begin() → yourAgent() → complete()</span>
                </div>
              </motion.div>
            </div>
            <div className="mono relative flex items-center gap-2 rounded-md border border-line px-2.5 py-1.5 text-[9.5px] text-ink-faint">
              ...
              <motion.span style={{ opacity: caret }} className="-ml-1 inline-block h-[1em] w-px bg-ink" />
              <motion.span style={{ opacity: useTransform(run, (v) => (v > 0.02 ? 0 : 1)) }} className="absolute left-2.5">{placeholder}</motion.span>
              <Sparkles className="ml-auto size-3" strokeWidth={1.75} />
            </div>
          </div>
        </Tile>
        {/* scene 3: hot-reload toast */}
        <motion.div style={{ opacity: toastOp }} className="mono absolute bottom-10 left-1/2 -translate-x-1/2 rounded-full border border-settle/40 bg-paper px-2.5 py-1 text-[9px] text-settle shadow-sm">
      ...
      <Part p={p} from={{ x: 340, r: -5 }} win={[0.56, 0.7]} style={{ left: rightX, top: r2y, width: rightW, height: opsH }}>
        <Tile title="Operations" icon={Wrench} src="jobs · audit · admin">
          <MRow l="audit" r={useTransform(() => "execution.settled · 14 cr")} color={useTransform(() => "var(--settle)")} opacity={auditNew} />
...
function BoundaryPanel({ run }: { run: MotionValue<number> }) {
  ...
  const active = useTransform<number, string>(run, (v) => (v < 0.2 ? "none" : v < 0.42 ? "admit" : v < 0.68 ? "run" : "settle"));
  return (
    <motion.div style={{ x, opacity, left: rightX, top: r2y, width: rightW, height: r2h }} className="absolute flex flex-col overflow-hidden rounded-md border border-line-strong bg-paper shadow-[0_24px_60px_-30px_rgba(0,0,0,0.5)] will-change-transform">
      ...
      <div className="mono border-t border-line px-2.5 py-1.5 text-[8.5px] text-ink-faint">
        <motion.span>{useTransform<string, string>(active, (a) => (a === "admit" ? "ADMIT · plan checked through a port, worst-case cost held" : a === "run" ? "RUN · no wrapper, no agent API — the handle knows nothing about messages" : a === "settle" ? "SETTLE · tokens and cost recorded, idempotent — no double billing" : "waiting for a message"))}</motion.span>
      </div>
    </motion.div>
  );
}

function CodeLine({ l, active }: { l: (typeof CODE)[number]; active: MotionValue<string> }) {
  const bg = useTransform(active, (a) => (a === l.step ? "var(--accent-soft)" : "transparent"));
  const color = useTransform(active, (a) => (a === "none" || a === l.step ? "var(--ink)" : "var(--ink-faint)"));
  return (
    <motion.div style={{ background: bg, color }} className="-mx-1 whitespace-pre rounded-sm px-1">
      {l.text || " "}
    </motion.div>
  );
}
...
function Editor({ yours, time }: { yours: MotionValue<number>; time: MotionValue<number> }) {
  const EW = 390;
  ...
  const oldName = useTransform(yours, (v) => "Assistant".slice(0, Math.round((1 - seg(v, 0.22, 0.3)) * 9)));
  const tagline = useTyped(yours, 0.5, 0.6, "Ask about any clause");
  const oldTag = useTransform(yours, (v) => "Ask anything".slice(0, Math.round((1 - seg(v, 0.44, 0.5)) * 12)));
  const modified = useTransform(yours, [0.6, 0.64], [0, 1]);
  return (
    <motion.div style={{ x, opacity, left: 0, top: 20, width: EW, height: H - 40 }} className="absolute flex overflow-hidden rounded-lg border border-line-strong bg-paper shadow-[0_24px_60px_-30px_rgba(0,0,0,0.5)] will-change-transform">
      <div className="mono w-[150px] shrink-0 border-r border-line bg-paper-sunken/60 p-2.5 text-[9px]">
        <div className="mb-2 text-ink-faint">my-app · installed by shadcn add</div>
        {TREE.map(([f, hot]) => (
          <div key={f} className={cn("flex items-center justify-between truncate rounded-sm px-1 py-[3px]", hot ? "bg-paper text-ink" : "text-ink-dim")}>
            <span className="truncate">{f}</span>
            {hot && <motion.span style={{ opacity: modified }} className="text-amber">M</motion.span>}
          </div>
        ))}
        <div className="mt-3 border-t border-line pt-2 text-ink-faint">consumer-owned · not a dependency</div>
      </div>
      <div className="mono flex-1 p-3 text-[10px] leading-[1.65] text-ink">
        <div className="mb-2 text-ink-faint">lib/chat-config.tsx</div>
        <div><span className="text-amber">export const</span> chatConfig = {"{"}</div>
        <div className="pl-3">agent: {"{"}</div>
        <div className="pl-6">name: <span className="text-settle">"<motion.span>{oldName}</motion.span><motion.span>{name}</motion.span>"</span>,</div>
        <div className="pl-6">tagline: <span className="text-settle">"<motion.span>{oldTag}</motion.span><motion.span>{tagline}</motion.span>"</span>,</div>
        <div className="pl-3">{"},"}</div>
        <div className="pl-3">starters: [<span className="text-settle">"Summarise this contract"</span>, <span className="text-settle">"Flag renewal risks"</span>],</div>
        <div className="pl-3">autoContinue: <span className="text-amber">true</span>,</div>
        <div>{"};"}</div>
        <div className="mt-4 text-ink-faint">// components/chat/* — unchanged</div>
        <div className="text-ink-faint">// re-install the item later: your config survives</div>
      </div>
    </motion.div>
  );
}
...
const UPGRADE: Line[] = [
  ...
  { text: `@intelligo-dev/* ${PACKAGE_VERSION} → ${NEXT_VERSION} · 12 generated files compared`, tone: "dim" },
  ...
  { text: "10 current · 2 customized · 0 conflict — nothing overwritten", tone: "ok" },
];
...
function UpgradeTerminal({ time }: { time: MotionValue<number> }) {
  ...
  return (
    <motion.div style={{ y, opacity, left: rightX, top: r2y + opsH + gap, width: rightW, height: r2h - opsH - gap }} className="absolute flex flex-col overflow-hidden rounded-md border border-line-strong bg-paper-sunken shadow-[0_24px_60px_-30px_rgba(0,0,0,0.5)] will-change-transform">
      <div className="mono flex items-center gap-1.5 border-b border-line px-2.5 py-1 text-[9px] text-ink-faint">
        <span className="size-1.5 rounded-full bg-line-strong" /><span className="size-1.5 rounded-full bg-line-strong" /><span className="size-1.5 rounded-full bg-line-strong" />
      ...
      <div className="mono flex-1 space-y-px p-2.5 text-[9px] leading-[1.5]">
        {UPGRADE.map((l, i) => {
          ...
          return <UpgradeLine key={i} line={l} opacity={useTransform(time, [start, start + 0.02], [0, 1])} />;
        })}
      </div>
    </motion.div>
  );
...
function UpgradeLine({ line, opacity }: { line: Line; opacity: MotionValue<number> }) {
  return (
    <motion.div style={{ opacity }} className={cn("whitespace-pre-wrap break-words", line.tone === "cmd" && "text-ink", line.tone === "ok" && "text-settle", line.tone === "dim" && "text-ink-dim", line.tone === "amber" && "text-amber")}>
      {line.tone === "cmd" && <span className="text-ink-faint">$ </span>}
      {line.text}
    </motion.div>
  );
...
function App({ build, run, yours, time }: { build: MotionValue<number>; run: MotionValue<number>; yours: MotionValue<number>; time: MotionValue<number> }) {
  ...
  const scale = useTransform(() => shrinkIn.get() + (1 - shrinkIn.get()) * shrinkOut.get());
  return (
    <motion.div style={{ opacity }} className="absolute inset-0">
      <motion.div style={{ scale, transformOrigin: "right center" }} className="absolute inset-0 will-change-transform">
        ...
        <div className="grid gap-6 lg:grid-cols-[250px_1fr] lg:gap-10">
          <div className="order-2 lg:order-1">
            ...
            <div className="mt-5 hidden lg:block">
              <div className="mono flex items-center justify-between text-[0.68rem] uppercase tracking-[0.08em] text-ink-faint">
                <span>scene {SCENES[scene]!.n} / 0{SCENES.length}</span>
                <motion.span className="tabular-nums">{pct}</motion.span>
              </div>
              <div className="mt-1 text-[1.05rem] font-semibold text-ink">{SCENES[scene]!.title}</div>
              <p className="mt-2 text-[0.88rem] text-ink-dim">{SCENES[scene]!.note}</p>
              <motion.p style={{ opacity: done }} className="mono mt-4 text-[0.72rem] text-settle">
                ✓ every part but one is on npm or in the registry. The one is yours.
              </motion.p>
            </div>
          </div>
          <div ref={ref} className="order-1 overflow-hidden lg:order-2">
            ...
            <p className="mono mt-3 text-center text-[0.7rem] text-ink-faint lg:hidden">
              <span className="text-ink-dim">{SCENES[scene]!.title}</span> · {STEPS[step]!.label}
            </p>
          </div>
        </div>
      </div>
...
function Rail({ scene, step, className }: { scene: number; step: string; className?: string }) {
  return (
    <ol className={cn("flex flex-wrap gap-1 lg:flex-col lg:gap-0 lg:border-t lg:border-line", className)} aria-label="Scenes">
      {SCENES.map((s, i) => (
        <li
          key={s.id}
          className={cn(
            ...
            i < scene ? "text-ink-dim" : i === scene ? "border-amber text-ink lg:border-line" : "text-ink-faint"
          )}
        >
          <span className="flex items-center gap-2">
            <span className={cn("inline-block size-1.5 rounded-full", i < scene ? "bg-settle" : i === scene ? "bg-amber" : "bg-line-strong")} />
            <span className="text-ink-faint">{s.n}</span>
            <span>{s.title}</span>
          </span>
          {i === scene && <span className="mt-1 hidden pl-[1.6rem] text-[0.7rem] text-amber lg:block">{step}</span>}
        </li>
      ))}
    </ol>
  );
