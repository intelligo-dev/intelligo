...
Prefer to explore first? Clone this repository, run `pnpm install && pnpm dev`, and open **`apps/app`**, the reference application: a complete generic workspace AI SaaS built from nothing but the public packages and installed registry items. Nobody writes it by hand — it is exactly what the two commands above produce, and CI recreates it from a clean scaffold on every run — so it is the place to see a finished install, not the place to start your product. Yours begins with `create`, in your own repository.
...
apps/app           the reference application: what `intelligo create` + `shadcn add` produce, regenerated by CI — read it, don't build in it
apps/website       intelligo.dev — the public site and docs, which also serves the registry at /r
tools/film         the one-minute project film (Remotion); renders the video the site and this README show
