...
function LedgerLine({
  ...
  const opacity = useTransform(progress, [start, end], reduce ? [1, 1] : [0.12, 1]);
  ...
  return (
    <motion.li style={{ opacity, x }} className="flex items-baseline gap-3 border-b border-line py-2 text-[0.92rem]">
      <span className="mono w-5 shrink-0 text-[0.7rem] tabular-nums text-ink-faint">{String(index + 1).padStart(2, "0")}</span>
      <motion.span style={{ color }} className="flex-1">{text}</motion.span>
      <span className="mono hidden text-[0.68rem] text-ink-faint sm:inline">{pkg}</span>
      <motion.span style={{ opacity: checkOpacity }} className="mono text-[0.7rem] text-settle">✓ shipped</motion.span>
    </motion.li>
  );
}
...
export function ScrollLedger() {
  ...
  const counted = useTransform(scrollYProgress, [0, 0.72], [0, OTHER_HALF.length]);
  ...
  const finalY = useTransform(scrollYProgress, [0.82, 0.92], reduce ? [0, 0] : [8, 0]);
    ...
    <div ref={ref} className="grid gap-10 lg:grid-cols-[1fr_1.35fr] lg:gap-16">
      ...
      <div className="lg:sticky lg:top-24 lg:self-start">
        ...
        <ul className="mt-4 border-t border-line-strong">
          {YOUR_HALF.map((y) => (
            <li key={y.title} className="border-b border-line py-4">
              <div className="text-[1.35rem] font-semibold leading-tight text-ink">{y.title}</div>
        ...
        <div className="mt-6 border border-line-strong bg-paper-raised p-4">
          <div className="mono flex items-baseline gap-2 text-[0.72rem] text-ink-faint">
            ...
            <motion.span className="ml-auto text-[1.6rem] leading-none tabular-nums text-ink">{counterText}</motion.span>
          </div>
          <motion.p style={{ opacity: finalOpacity, y: finalY }} className="mt-3 text-[0.95rem] text-ink">
            {OTHER_HALF.length} things, none of them yours. <strong className="text-settle">All shipped.</strong>
          </motion.p>
        </div>
      </div>
      ...
      <div>
        ...
        <ol className={cn("mt-4 border-t border-line-strong")}>
          {OTHER_HALF.map((o, i) => (
            <LedgerLine key={o.text} index={i} total={OTHER_HALF.length} progress={scrollYProgress} text={o.text} pkg={o.pkg} reduce={reduce} />
          ))}
        </ol>
      </div>
    </div>
